...
class SnapshotServiceImplTest extends Specification {
    ...
    def "test querySnapshot with snapshotTs null should return current snapshot"() {
        ...
        then:
        1 * entityService.getEntityByCode(ENTITY_CODE, ENTITY_ID) >> entity
        1 * metricService.getMetricMetaDaoByCode(METRIC_CODE_1) >> metricMetaDAO
    ...
    def "test querySnapshot with multiple metrics should return all metrics"() {
        ...
        1 * metricService.getMetricMetaDaoByCode(METRIC_CODE_2) >> metricMetaDAO2
    ...
    def "test querySnapshot with metric not found in service should filter out"() {
        ...
        then:
        1 * entityService.getEntityByCode(ENTITY_CODE, ENTITY_ID) >> entity
        1 * metricService.getMetricMetaDaoByCode(METRIC_CODE_1) >> null
        0 * cacheService.get(_, _, _, _, _)
        result != null
